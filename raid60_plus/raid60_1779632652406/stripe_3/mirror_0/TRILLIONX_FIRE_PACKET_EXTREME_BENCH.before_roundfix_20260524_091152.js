...
const MB=x=>r(x/1048576),GB=x=>r(x/1073741824),TB=x=>r(x/1099511627776);
...
if(!isMainThread){
 ...
 parentPort.postMessage({ms:r(performance.now()-t0),loops,loops_s:r(loops/((performance.now()-t0)/1000)),hash:h.slice(0,16),acc:r(acc)});
 return;
}
function host(){
 ...
 return {
  ...
  measured_cpu_ghz:r(ghz),
  logical_aggregate_ghz:r(ghz*lc),
  logical_aggregate_thz:r((ghz*lc)/1000),
  logical_cpus:lc,
...
function matrix(n){
 ...
 for(let i=0;i<n*n;i++){A[i]=(i%251)/251;B[i]=(i%239)/239}
 let t=performance.now();
 for(let i=0;i<n;i++)for(let k=0;k<n;k++){let a=A[i*n+k];for(let j=0;j<n;j++)C[i*n+j]+=a*B[k*n+j]}
 let ms=performance.now()-t,gops=(2*n*n*n)/(ms/1000)/1e9;
 return{n,ms:r(ms),gops:r(gops),tops:r(gops/1000),checksum:r(C[(n*n/2)|0])}
}
function memstream(mb,rounds){
 let n=Math.floor(mb*1048576/8),a=new Float64Array(n),b=new Float64Array(n);for(let i=0;i<n;i++)a[i]=i%1009;
 let t=performance.now();for(let q=0;q<rounds;q++)for(let i=0;i<n;i++)b[i]=a[i]*1.000001+b[i]*0.000001;
 let ms=performance.now()-t,gbps=(mb*1048576*rounds*2/(ms/1000))/1073741824;
 return{mb,gb:r(mb/1024),rounds,ms:r(ms),gb_s:r(gbps),tb_s:r(gbps/1024)}
}
function hash(mb,rounds){
 let b=crypto.randomBytes(mb*1048576),h="",t=performance.now();
 for(let i=0;i<rounds;i++)h=crypto.createHash("sha256").update(b).update(String(i)).digest("hex");
 let ms=performance.now()-t,mbps=mb*rounds/(ms/1000);
 return{mb_total:mb*rounds,ms:r(ms),mb_s:r(mbps),gb_s:r(mbps/1024),hash:h.slice(0,16)}
}
function comp(mb){
 ...
 return{mb,ms:r(ms),mb_s:r(mbps),gb_s:r(mbps/1024),ratio:r(gz.length/b.length),ok:out.length===b.length}
}
function bigint(bits,rounds){
 ...
 for(let i=0;i<rounds;i++)x=(x*a+b)&m;
 let ms=performance.now()-t;return{bits,rounds,ms:r(ms),ops_s:r(rounds/(ms/1000)),tail:x.toString(16).slice(-16)}
}
function fftlike(n,rounds){
 let re=new Float64Array(n),im=new Float64Array(n);for(let i=0;i<n;i++){re[i]=Math.sin(i);im[i]=Math.cos(i)}
 let t=performance.now(),acc=0;
 for(let q=0;q<rounds;q++)for(let step=1;step<n;step<<=1){let jump=step<<1;for(let k=0;k<step;k++){let ang=-Math.PI*k/step,wr=Math.cos(ang),wi=Math.sin(ang);for(let i=k;i<n;i+=jump){let j=i+step,tr=wr*re[j]-wi*im[j],ti=wr*im[j]+wi*re[j];re[j]=re[i]-tr;im[j]=im[i]-ti;re[i]+=tr;im[i]+=ti;acc+=re[i]*1e-12}}}
 let ms=performance.now()-t;return{n,rounds,ms:r(ms),points_s:r((n*rounds)/(ms/1000)),checksum:r(acc)}
}
function sortjson(items){
 let arr=[];for(let i=0;i<items;i++)arr.push({i,v:crypto.randomBytes(8).toString("hex"),x:Math.random()});
 ...
 return{items,serialized_mb:MB(Buffer.byteLength(s)),ms:r(ms),items_s:r(items/(ms/1000)),ok:p.length===items}
}
async function workers(w,loops,mb){
 let t=performance.now(),jobs=[];for(let i=0;i<w;i++)jobs.push(new Promise(res=>{let ww=new Worker(__filename,{workerData:{loops,mb}});let tm=setTimeout(()=>{try{ww.terminate()}catch(e){}res({timeout:true})},25000);ww.on("message",m=>{clearTimeout(tm);res(m)});ww.on("error",e=>{clearTimeout(tm);res({error:e.message})})}));
 let out=await Promise.all(jobs),ms=performance.now()-t,ok=out.filter(x=>!x.error&&!x.timeout).length,ls=(w*loops)/(ms/1000);
 return{workers:w,ok,failed:w-ok,ms:r(ms),loops_s:r(ls),mloops_s:r(ls/1e6),detail:out}
}
function cfg(){
 let a=[];for(let p=1;p<=10;p++)a.push({packet:p,matrix:96+p*20,mem:24+p*12,memr:2+Math.floor(p/2),hash:16+p*4,hashr:3+p,comp:8+p*3,bits:1024+p*1024,bigr:4000+p*2500,fft:1024*(1+Math.floor(p/2)),fftr:1+Math.floor(p/3),json:1500+p*1200,w:Math.min(4,2+Math.floor(p/3),os.cpus().length),loops:600000+p*450000,wmb:1+Math.floor(p/3)});return a}
function score(x){
 let a=x.results;return r(x.packet*10+Math.min(35,a.matrix.gops*30)+Math.min(25,a.memory.gb_s*3)+Math.min(30,a.hash.mb_s/45)+Math.min(15,a.compression.mb_s/20)+Math.min(25,a.workers.mloops_s)+Math.min(10,a.bigint.ops_s/2000)+Math.min(10,a.fftlike.points_s/250000)+Math.min(10,a.sortjson.items_s/5000));
}
function health(x){
 ...
 return{score:r(h),class:h>=90?"EXCELLENT":h>=75?"GOOD":h>=60?"PRESSURE":"LIMIT",flags:f}
}
(async()=>{
 ...
 for(const c of cfg()){
  ...
  let item={packet:c.packet,config:c,ms:r(performance.now()-t0),event_p95_ms:r(mon.percentile(95)/1e6),before,after:memnow(),results};
